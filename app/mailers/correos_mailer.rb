...
class CorreosMailer < ActionMailer::Base
  ...
  def enviar_correo(deQuien,correo,de,adjuntos=nil,aquienSeAEnviado)
    @correo = correo
    if not aquienSeAEnviado.include?(de)
      if adjuntos != nil
      ...
      mail(:to => "kailer89@gmail.com", :subject => "#{correo.nombredelmail}",:from=>deQuien)
