class InscritosController < ApplicationController
  ...
  def index
    ...
    if modelo == nil           
    @inscritos = Compartido.getInscritosForUser(current_user).order(sort_column + " " + sort_direction).paginate(:per_page => 15, :page => params[:page])
    else
      @inscritos = Compartido.getInscritosForUser(current_user).where(:archivado=>archivado).order(sort_column + " " + sort_direction).paginate(:per_page => 15, :page => params[:page])
    end
    ...
    if rol.nombre == "DN" or rol.nombre == "ACRM"
      ...
      @inscritos = Compartido.getInscritosForUser(current_user)
    end
